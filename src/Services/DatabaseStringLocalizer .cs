...
/// </summary>
/// <remarks>This class implements the IStringLocalizer interface to enable multi-tenant, culture-specific
...
/// tenant and culture, and do not fall back to parent cultures automatically.</remarks>
public class DatabaseStringLocalizer : IStringLocalizer
{
    ...
    /// </summary>
    /// <remarks>If a translation for the specified resource name does not exist for the current tenant and
    /// culture, the returned <see cref="LocalizedString"/> will have its <c>ResourceNotFound</c> property set to <see
    /// langword="true"/> and its value will be the resource name itself.</remarks>
    /// <param name="name">The name of the resource to retrieve the localized string for.</param>
    /// <returns>A <see cref="LocalizedString"/> containing the localized value if found; otherwise, a <see
    /// cref="LocalizedString"/> containing the resource name as the value and indicating that the resource was not
    /// found.</returns>
    ...
    /// <param name="arguments">An array of objects to format the string with, if the resource supports formatting. Can be empty.</param>
    /// <returns>A <see cref="LocalizedString"/> representing the localized resource. If the resource is not found, returns a
    /// <see cref="LocalizedString"/> indicating the missing resource.</returns>
    ...
    /// </summary>
    /// <param name="includeParentCultures">true to include strings from parent cultures in addition to the current culture; otherwise, false.</param>
    /// <returns>An enumerable collection of LocalizedString objects representing all available localized strings for the current
    /// culture and tenant.</returns>
