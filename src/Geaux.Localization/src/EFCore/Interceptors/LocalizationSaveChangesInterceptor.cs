namespace Geaux.Localization.EFCore.Interceptors;
...
public sealed class LocalizationSaveChangesInterceptor : SaveChangesInterceptor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            PropertyInfo[] props = entry.Entity.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance);
    ...
    private async Task UpsertLocalizationAsync(string? tenantId, string culture, string key, string value, CancellationToken cancellationToken)
    {
        ...
        // Ensure key exists
        LocalizationKey? keyEntity = await _localizationDb.LocalizationKeys
        ...
        LocalizationValue? existing = await _localizationDb.LocalizationValues
